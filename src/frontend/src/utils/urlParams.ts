...
/**
 * Gets the stored redirect path from sessionStorage (set by 404.html on deep-link).
 * Returns null if no redirect is pending.
 */
export function getRedirectPath(): string | null {
  return getSessionParameter(REDIRECT_PARAM);
}
...
export function clearRedirectParam(): void {
  clearSessionParameter(REDIRECT_PARAM);
}
